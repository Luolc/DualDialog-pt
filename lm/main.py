...
parser.add_argument('--data', type=str, default='./data/daily',
...
def batchify(dataset, bsz, bptt):
    # Work out how cleanly we can divide the dataset into bsz parts.
    nbatch = dataset.size(0) // bsz
    nbatch //= bptt
    # Trim off any extra elements that wouldn't cleanly fit (remainders).
    dataset = dataset.narrow(0, 0, nbatch * bsz * bptt)
...
model = model.RNNModel(args.model, len(corpus.dictionary), args.emsize, args.nhid, args.nlayers,
...
def evaluate(data_source):
    ...
    with torch.no_grad():
        for i in range(0, data_source.size(0) - 1, args.bptt):
            ...
            batch_data, targets = get_batch(data_source, i)
            output, _ = model(batch_data, hidden)
            output_flat = output.view(-1, ntokens)
            total_loss += len(batch_data) * criterion(output_flat, targets).item()
...
def train():
    ...
    for batch, i in enumerate(range(0, train_data.size(0) - 1, args.bptt)):
        ...
        batch_data, targets = get_batch(train_data, i)
        ...
        if batch % args.log_interval == 0 and batch > 0:
            ...
            nbatches = len(train_data) // args.bptt
            print('| epoch {:3d} | {:5d}/{:5d} batches | lr {:02.2f} | ms/batch {:5.2f} | '
                  'loss {:5.2f} | ppl {:8.2f}'.format(epoch, batch, nbatches, lr,
